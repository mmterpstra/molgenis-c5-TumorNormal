#PBS -N AddOrReplaceReadGroups_1
#PBS -q gcc
#PBS -l nodes=1:ppn=4
#PBS -l walltime=23:59:00
#PBS -l mem=6gb
#PBS -e AddOrReplaceReadGroups_1.err
#PBS -o AddOrReplaceReadGroups_1.out
#PBS -W umask=0007
...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_AddOrReplaceReadGroups_1_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'AddOrReplaceReadGroups_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
port="80"
interval="2000"
path="."source $ENVIRONMENT_DIR/BwaMemAlignment_1.env
...
chmod 755 $ENVIRONMENT_DIR/AddOrReplaceReadGroups_1.env
...
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task AddOrReplaceReadGroups_1 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
