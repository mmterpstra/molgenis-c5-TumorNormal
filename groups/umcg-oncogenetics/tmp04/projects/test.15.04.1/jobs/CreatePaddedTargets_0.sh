#PBS -N CreatePaddedTargets_0
#PBS -q gcc
#PBS -l nodes=1:ppn=1
#PBS -l walltime=23:59:00
#PBS -l mem=4
#PBS -e CreatePaddedTargets_0.err
#PBS -o CreatePaddedTargets_0.out
#PBS -W umask=0007
...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_CreatePaddedTargets_0_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'CreatePaddedTargets_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
chmod 755 $ENVIRONMENT_DIR/CreatePaddedTargets_0.env
...
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task CreatePaddedTargets_0 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
