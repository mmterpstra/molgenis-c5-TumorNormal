#PBS -N MergeBamFiles_0
#PBS -q gcc
#PBS -l nodes=1:ppn=4
#PBS -l walltime=23:59:00
#PBS -l mem=6gb
#PBS -e MergeBamFiles_0.err
#PBS -o MergeBamFiles_0.out
#PBS -W umask=0007
...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_MergeBamFiles_0_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'MergeBamFiles_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/MergeBamFiles_0.sh.started
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'MergeBamFiles_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
port="80"
interval="2000"
path="."source $ENVIRONMENT_DIR/AddOrReplaceReadGroups_0.env
...
MergeBamFilesBam="/gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bam"
WORKDIR="/gcc/"
stage="module load"
...
MergeBamFilesBai="/gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bai"
...
alloutputsexist \
"/gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bam" \
"/gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bai"
...
java -Xmx6g -XX:ParallelGCThreads=4 -jar $PICARD_HOME/picard.jar MergeSamFiles \
 ...
 OUTPUT=/gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bam \

# VALIDATION_STRINGENCY=LENIENT \

putFile /gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bam
putFile /gcc/groups/umcg-oncogenetics/tmp01/projects/test//MergeBams/samplePE.bai
...
# Save output in environment file: '$ENVIRONMENT_DIR/MergeBamFiles_0.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/MergeBamFiles_0.env
chmod 755 $ENVIRONMENT_DIR/MergeBamFiles_0.env
...
touch $ENVIRONMENT_DIR/MergeBamFiles_0.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task MergeBamFiles_0 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
...
# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task MergeBamFiles_0.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log