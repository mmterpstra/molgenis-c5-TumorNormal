#PBS -N BwaMemAlignment_1
#PBS -q gcc
#PBS -l nodes=1:ppn=8
#PBS -l walltime=10:00:00
#PBS -l mem=8gb
#PBS -e BwaMemAlignment_1.err
#PBS -o BwaMemAlignment_1.out
#PBS -W umask=0007
...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_BwaMemAlignment_1_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'BwaMemAlignment_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/BwaMemAlignment_1.sh.started
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'BwaMemAlignment_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
alloutputsexist \
	/gcc/groups/umcg-oncogenetics/tmp01/projects/test//bwa/2_sampleSE.sam
...
putFile /gcc/groups/umcg-oncogenetics/tmp01/projects/test//bwa/2_sampleSE.sam 
...
# Save output in environment file: '$ENVIRONMENT_DIR/BwaMemAlignment_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/BwaMemAlignment_1.env
chmod 755 $ENVIRONMENT_DIR/BwaMemAlignment_1.env
...
touch $ENVIRONMENT_DIR/BwaMemAlignment_1.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task BwaMemAlignment_1 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
...
# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task BwaMemAlignment_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log