#MOLGENIS nodes=1 ppn=6 mem=8gb walltime=10:00:00
...
if [ ${#reads3FqGz} -eq 0 ]; then
	...
	if [ ${#reads2FqGz} -eq 0 ]; then
		python $EBROOTNUDUPPY/nudup.py -o ${nugeneBam} -l 6 ${addOrReplaceReadGroupsBam}
	else
		python $EBROOTNUDUPPY/nudup.py -2 -o ${nugeneBam} -l 6 ${addOrReplaceReadGroupsBam}
	fi
