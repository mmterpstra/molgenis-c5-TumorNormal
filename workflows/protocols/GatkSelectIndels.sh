...
java -Xmx4g -Djava.io.tmpdir=${variantFiltDir} \
 ...
 --filterExpression "vc.hasAttribute('FS') && (!(vc.getAttribute('FS').toString().equals('.'))) && FS > 200.0"  --filterName "FSgt200" \
 --filterExpression "vc.hasAttribute('RPA') && (!(vc.getAttribute('RPA').toString().equals('.'))) &&(vc.getAttribute('RPA').0 > 8||vc.getAttribute('RPA').1 > 8||vc.getAttribute('RPA').2 > 8)"  --filterName "RPAgt8" \
