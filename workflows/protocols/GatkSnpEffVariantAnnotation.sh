#MOLGENIS walltime=33:59:00 mem=9gb ppn=1 tmp=50gb
...
if [ "z$TMPDIR" == "z" ]; then 
	...
	DBNSFP=$LOCALTEMP/$(basename "${dbnsfp}")

	CLINVARVCF="${clinvarVcf}"
	
	ONEKGVCF="${oneKgPhase1SnpsVcf}"
	
	GNOMADVCF="${gnomadVcf}"

	GNOMAD312VCF="${gnomad312Vcf}"

	SNPEFFDIR=$LOCALTEMP/$(basename "${snpeffDataDir}")
else
	...
	DBSNP=$LOCALTEMP/$(basename "${dbsnpVcf}")
	cp -v ${dbsnpVcf}* $LOCALTEMP

	DBNSFP=$LOCALTEMP/$(basename "${dbnsfp}")
	cp -v ${dbnsfp}* $LOCALTEMP
	

	CLINVARVCF=$LOCALTEMP/$(basename "${clinvarVcf}")
	cp -v ${clinvarVcf}* $LOCALTEMP
	
	ONEKGVCF=$LOCALTEMP/$(basename "${oneKgPhase1SnpsVcf}")
	cp -v ${oneKgPhase1SnpsVcf}* $LOCALTEMP
	
	GNOMADVCF=$LOCALTEMP/$(basename "${gnomadVcf}")
	cp -v ${gnomadVcf}* $LOCALTEMP

	GNOMAD312VCF=$LOCALTEMP/$(basename "${gnomad312Vcf}")
	cp -v ${gnomad312Vcf}* $LOCALTEMP

	SNPEFFDIR=$LOCALTEMP/$(basename "${snpeffDataDir}")
	cp -vr ${snpeffDataDir} $LOCALTEMP
fi
...
(
	...
	#GRCh38.86 For one of the genes (idk what gene ) there is a start loss mutation incorparated in one of the default transcripts and this is taken as the least harmful mutation due to the gene being a processed transcript and not protein coding. 
	java -Xmx8g -jar  $EBROOTSNPEFF/snpEff.jar \
	 -c $EBROOTSNPEFF/snpEff.config \
	 -dataDir $SNPEFFDIR \
	 -hgvs \
...
(
	...
	java -Xmx8g -jar $EBROOTSNPEFF/SnpSift.jar annotate \
	 $GNOMADVCF \
	 ${annotVcf}.snpsiftdbnsfp.vcf > ${annotVcf}.snpsiftannot.tmp.vcf

	java -Xmx8g -jar $EBROOTSNPEFF/SnpSift.jar annotate \
	 $CLINVARVCF \
	 ${annotVcf}.snpsiftannot.tmp.vcf > ${annotVcf}.snpsiftannot.tmp2.vcf 

	java -Xmx8g -jar $EBROOTSNPEFF/SnpSift.jar annotate \
	 $GNOMAD312VCF \
	 ${annotVcf}.snpsiftannot.tmp2.vcf > ${annotVcf}.snpsiftannot.vcf
