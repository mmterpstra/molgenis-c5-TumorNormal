...
sub GetWorstAnnByAllele {
	...
	#my $spanningDeletionAnn=$anns[0];
	#my $count = $spanningDeletionAnn =~ tr/\|//;
	#$spanningDeletionAnn = '*'.'|.' x $count;
	...
	#what is the implicit rule here first is worse...
	while(not(defined($worstAnn)) && scalar(@anns) > 0){
		my $ann = shift @anns;
